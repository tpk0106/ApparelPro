...
{
    // Modern equivalent of legacy GI_STVAL.PRG's printed report.
    public static class GeneralStockSummaryReportEngine
    {
        public static byte[] GenerateStockSummaryReportPdf(
            GeneralStockSummaryReportHeaderServiceModel header,
            List<GeneralStockSummaryReportLineServiceModel> lines)
        {
            ...
            Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    {
                        column.Item().AlignCenter().Text("Summary of Stock Value").FontSize(14).Bold();
                        column.Item().PaddingTop(5).Text($"As at {monthLabel}    Currencies : {header.Currency1} / {header.Currency2}");
                    ...
                    page.Content().PaddingTop(10).Table(table =>
                    {
                        ...
                        foreach (var line in lines)
                        {
                            ...
                            {
                                case "Store":
                                    table.Cell().Padding(3).PaddingLeft(15).Text($"{line.StoreCode} - {line.StoreDescription}");
