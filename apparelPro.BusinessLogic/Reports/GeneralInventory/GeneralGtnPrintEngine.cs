...
{
    // Modern equivalent of legacy GI_GGTN2.PRG (print run of a committed General
    // Inventory Goods Transfer Note). Same header-repeats / signature-once-at-end
    // structure as the other GeneralInventory print engines.
    public class GeneralGtnPrintEngine
    {
        public static byte[] GenerateGtnPrintPdf(GeneralGtnPrintDetailsServiceModel details)
        {
            ...
            Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header().Column(column =>
                    {
                        column.Item().Row(row =>
                        {
                            row.RelativeItem().Text("GOODS TRANSFER NOTE (General Inventory)").FontSize(14).Bold();
                        ...
                        column.Item().PaddingTop(2).Text($"GTN No : {details.Header.GtnNumber}").Bold();

                        column.Item().PaddingTop(6).LineHorizontal(1).LineColor(Colors.Black);

                        column.Item().PaddingTop(6).Row(row =>
                        {
                            row.RelativeItem().Text(t => { t.Span("From Stores : ").Bold(); t.Span(details.Header.FromStoreDescription); });
                            row.RelativeItem().Text(t => { t.Span("To Stores : ").Bold(); t.Span(details.Header.ToStoreDescription); });
                        });
