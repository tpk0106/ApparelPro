...
{
    // Modern equivalent of legacy IN_SRN2.PRG (print run of an already-committed
    // Supplier Return Note). See NotePrintEngineHelper for the shared
    // header/signature/footer scaffolding shared with every note print engine.
    public class SrnPrintEngine
    {
        public static byte[] GenerateSrnPrintPdf(SrnPrintDetailsServiceModel details)
        {
                    ...
                    page.Content().PaddingTop(10).Column(column =>
                    {
                        column.Item().Table(table =>
                        {
                            table.ColumnsDefinition(columns =>
                            {
                                ...
                                columns.ConstantColumn(95);   // Qty. Returned
                            ...
                            table.Header(header =>
                            {
                                ...
                                header.Cell().PaddingHorizontal(4).AlignRight().Text("Qty. Returned").Bold();
