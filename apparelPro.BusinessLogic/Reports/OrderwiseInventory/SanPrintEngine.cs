...
{
    // Modern equivalent of legacy IN_SAN2.PRG (print run of an already-committed
    // Stock Adjustment Note). See NotePrintEngineHelper for the shared
    // header/signature/footer scaffolding shared with every note print engine.
    public class SanPrintEngine
    {
        public static byte[] GenerateSanPrintPdf(SanPrintDetailsServiceModel details)
        {
                    ...
                    page.Content().PaddingTop(10).Column(column =>
                    {
                        column.Item().Table(table =>
                        {
                            table.ColumnsDefinition(columns =>
                            {
                                ...
                                columns.ConstantColumn(105);  // Adjusted Qty.
                            ...
                            table.Header(header =>
                            {
                                ...
                                header.Cell().PaddingHorizontal(4).AlignRight().Text("Adjusted Qty.").Bold();
                            ...
                            foreach (var line in details.Lines)
                            {
                                ...
                                table.Cell().PaddingVertical(3).PaddingHorizontal(4).AlignRight().Text(line.AdjustedQuantity.ToString("N2"));
