...
{
    // Modern equivalent of legacy IN_GIN2.PRG (print run of an already-committed
    // Goods Issue Note). See NotePrintEngineHelper for the shared
    // header/signature/footer scaffolding shared with every note print engine.
    public class GinPrintEngine
    {
        public static byte[] GenerateGinPrintPdf(GinPrintDetailsServiceModel details)
        {
                ...
                container.Page(page =>
                {
                    ...
                    page.RenderNoteHeader(
                        ...
                        detailsColumn => detailsColumn.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem().Text(t => { t.Span("Against SRN : ").Bold(); t.Span(details.Header.SourceStrnNumber); });
                        }));
