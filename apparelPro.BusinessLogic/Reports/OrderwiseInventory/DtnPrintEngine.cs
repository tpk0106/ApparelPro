...
{
    // Modern equivalent of legacy IN_DTN2.PRG (print run of an already-committed Direct
    // Goods Transfer Note). See NotePrintEngineHelper for the shared header/signature/
    // footer scaffolding shared with every note print engine.
    public class DtnPrintEngine
    {
        public static byte[] GenerateDtnPrintPdf(DtnPrintDetailsServiceModel details)
        {
            ...
            Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.RenderNoteHeader(
                        "DIRECT GOODS TRANSFER NOTE",
                        "DTN No",
                        details.Header.DtnNumber,
