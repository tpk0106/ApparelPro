...
namespace apparelPro.BusinessLogic.Services.Implementation.OrderwiseInventory
{
    ...
    public class SupplierReturnNoteService : ISupplierReturnNoteService
    {
        ...
        public async Task<SrnPrintDetailsServiceModel> GetSrnPrintDetailsAsync(string srnNumber)
        {
            srnNumber = srnNumber.Trim();

            var transactionRows = await _apparelProDbContext.OrderwiseStockTransactions
                .AsNoTracking()
                .Where(t => t.DocumentNumber == srnNumber && t.TransactionType == "7S")
                .OrderBy(t => t.Id)
                .ToListAsync();
            ...
            var lines = transactionRows.Select(t =>
            {
                ...
                return new SrnPrintLineServiceModel
                {
                    ItemCode = itemCode,
                    Description = !string.IsNullOrWhiteSpace(description) ? description!.Trim() : "(No description available)",
                    Unit = t.Unit,
            ...
            int supplierCode = firstRow.SupplierCode ?? 0;
            var supplierName = await _apparelProDbContext.Suppliers
                .AsNoTracking()
                .Where(s => s.SupplierCode == supplierCode)
                .Select(s => s.Name)
                .FirstOrDefaultAsync();

            return new SrnPrintDetailsServiceModel
            {
                Header = new SrnPrintHeaderServiceModel
                {
                    SrnNumber = srnNumber,
                    ...
                    SupplierName = !string.IsNullOrWhiteSpace(supplierName) ? supplierName! : supplierCode.ToString(),
