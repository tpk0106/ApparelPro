using ApparelPro.Data.Models.ImportExport;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;

namespace ApparelPro.Data.Configurations.ImportExport
{
    public class DutyTaxCodeConfig : IEntityTypeConfiguration<DutyTaxCode>
    {
        public void Configure(EntityTypeBuilder<DutyTaxCode> entity)
        {
            entity.ToTable("DutyTaxCodes");
            entity.HasKey(e => e.Code);
            entity.Property(e => e.Code).HasColumnType("varchar(4)");
            entity.Property(e => e.Description).HasColumnType("varchar(30)").IsRequired();
        }
    }
}
